...
// Timestamps remain UTC. Only calendar-day comparisons use the configured IANA zone.
const today = () => calendarDay();
const addDays = (date, days) => new Date(new Date(`${date}T00:00:00.000Z`).getTime() + days * 86400000).toISOString().slice(0,10);
...
export function scheduleReview({ intervalIndex = -1, delayedSuccess = 0, lapses = 0, rating, covered, delayed, at = today() }) {
  ...
  const nextIdx = rating === 'hard' ? Math.max(0, intervalIndex)
    : rating === 'easy' && delayed && nextSuccess >= 2 ? intervalIndex + 2 : intervalIndex + 1;
...
export function reviewDashboard(db) {
  ...
  const totals = db.prepare(`SELECT (SELECT COUNT(*) FROM knowledge_points) AS total,
    ...
    COALESCE(SUM(CASE WHEN due_on < ? THEN 1 ELSE 0 END),0) AS overdue FROM review_items`).get(date, date);
...
const getAttemptRow = (db, id) => {
  const row = db.prepare(`SELECT a.*, k.category, k.prompt, k.reference, k.source, r.interval_index, r.delayed_success, r.lapses,
      r.due_on, r.last_review_at FROM review_attempts a JOIN knowledge_points k ON k.id = a.knowledge_id
...
export function completeReview(db, id, input = {}) {
  ...
  try {
    ...
    const delayed = !!attempt.last_review_at && calendarDay(new Date(attempt.last_review_at)) < currentDate;
